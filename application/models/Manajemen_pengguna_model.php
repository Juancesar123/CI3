...
class Manajemen_pengguna_model extends CI_Model {
	...
	public function view_all($usernow){
		$sql = "SELECT mu.`username`, mu.`level`, mub.`nama_lengkap`, mub.`jenis_kelamin`, mub.`foto_path`, mu.`id`, mub.`id` as `id_biodata` FROM `master_user` mu LEFT JOIN `master_user_biodata` mub ON mu.`id_master_biodata` = mub.`id` WHERE mu.`status` != \"DELETE\" AND mu.`username` != \"$usernow\"";
